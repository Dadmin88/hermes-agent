...
def test_linux_discovery_includes_launcher_entries(tmp_path, monkeypatch):
    """The variant-named entries the Electron app installs are removable."""
    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.setenv("XDG_DATA_HOME", str(tmp_path / "xdg"))

    paths = gu.packaged_gui_app_paths()
    applications = tmp_path / "xdg" / "applications"

    assert applications / "com.nousresearch.hermes.desktop" in paths
    assert applications / "com.nousresearch.hermes-light.desktop" in paths
    # Legacy entries older builds wrote stay removable.
    assert applications / "hermes.desktop" in paths
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    entry = tmp_path / "xdg" / "applications" / "com.nousresearch.hermes.desktop"
    entry.parent.mkdir(parents=True, exist_ok=True)
    entry.write_text("x", encoding="utf-8")

    refreshed: list[Path] = []
    monkeypatch.setattr(
        gu, "_refresh_desktop_databases", lambda d: refreshed.append(d) or ["kbuildsycoca6"]
    )
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    refreshed: list[Path] = []
    monkeypatch.setattr(gu, "_refresh_desktop_databases", lambda d: refreshed.append(d) or [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")
