...
// generated by the electron-builder infra.
// has @@EXEC@@/@@ICON@@ placeholders the runtime installer substitutes.
async function bakedLinuxDesktopEntry() {
  const { desktopFileName, generateLinuxDesktopEntry } = await import('./gen-linux-desktop-entry.mjs')
  return JSON.stringify({ fileName: desktopFileName(), content: await generateLinuxDesktopEntry() })
}
...
const define = isDev
  ...
  : {
      ...
      '__HERMES_LINUX_DESKTOP_ENTRY__': await bakedLinuxDesktopEntry(),
    }
