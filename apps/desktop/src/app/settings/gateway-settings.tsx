...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const commitConfig = async (apply: boolean, allowPlainTextToken = false): Promise<void> => {
    ...
    try {
      ...
      // The opt-in path runs inside ConfirmDialog's onConfirm, which keeps the
      // dialog open with an inline error when it throws — rethrow a readable
      // message there so a failed save cannot play the success beat.
  ...
  // Save/Apply entry point: defer to the opt-in dialog when the token would
  // land on disk unencrypted; confirm resumes with allowPlainTextToken.
  const requestCommit = async (apply: boolean): Promise<void> => {
    ...
    <SettingsContent bare={embedded}>
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          ...
          await commitConfig(plainTextConfirm.apply, true)
        }}
